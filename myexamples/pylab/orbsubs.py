...
def read_two_bodies_new(simdir,froot,GM):
    ...
    # normal and perpendicular to dr_hat direction between primary and secondary, pointing to secondary.
    # this vector points in direction of secondary's orbital motion
    
    v_x,v_y,v_z=crossprod_unit(no_x,no_y,no_z,1,0,0)
    # a vector in orbital plane that is perpendicular to xhat
    ...
    orbit_x = dotprod(dx_hat,dy_hat,dz_hat,np.squeeze(xrot2[:,0]),\
        np.squeeze(xrot2[:,1]),np.squeeze(xrot2[:,2]))
    # angle between long axis of secondary and vector w rotation
    orbit_y = dotprod(a_x,a_y,a_z,np.squeeze(xrot2[:,0]),\
        np.squeeze(xrot2[:,1]),np.squeeze(xrot2[:,2]))
